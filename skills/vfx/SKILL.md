...
## Prerequisites
pika MCP available, and **local `ffmpeg`** in the run environment (used to extract every frame and build the contact sheets). Two inputs from the user: **(1) a video clip** (a local file or a public URL) and **(2) the VFX change** they want made to it. A third input is optional: **(3) a resolution** — defaults to 1080p if not specified. Tools used: `mcp__plugin_pika_pika__upload_asset`, `mcp__plugin_pika_pika__probe_media`, `mcp__plugin_pika_pika__analyze_media`, `mcp__plugin_pika_pika__transcribe_audio`, `mcp__plugin_pika_pika__estimate_cost`, `mcp__plugin_pika_pika__generate_reference_video`, `mcp__plugin_pika_pika__task_status` (frame extraction + tiling is done locally with ffmpeg, not an MCP tool).
...
## Step 1 — Upload the clip (`mcp__plugin_pika_pika__upload_asset`)
...
1. Call `mcp__plugin_pika_pika__upload_asset` with `filename`, `mime_type` (`video/mp4` for mp4), and `size_bytes` (the file's byte size).
...
## Step 4 — Read EVERY sheet + understand the audio (`mcp__plugin_pika_pika__upload_asset`, `mcp__plugin_pika_pika__analyze_media`, `mcp__plugin_pika_pika__transcribe_audio`)
...
For **each** contact sheet from Step 3 (cover ALL of them — every frame is on a sheet, no exceptions): upload it with `mcp__plugin_pika_pika__upload_asset` (the sheets are images, so upload to get a URL), then call `mcp__plugin_pika_pika__analyze_media` on that sheet URL with a `query` that reads the **full progression across those 25 frames** — pulling out everything the prompt must lock:
...
- `mcp__plugin_pika_pika__transcribe_audio` for **speech + timestamps** (the words and when they're said), so the prompt can preserve dialogue and lip-sync beats.
- `mcp__plugin_pika_pika__analyze_media` on the audio (or the video URL) to understand the **NON-speech audio too** — music, sound effects, ambience, tone, and rhythm/beats.
...
## Step 6 — Agreement gate (`mcp__plugin_pika_pika__estimate_cost`) — MANDATORY before generating
The render is the expensive step, and once fired it can't be un-spent. **Never call `mcp__plugin_pika_pika__generate_reference_video` without explicit user approval in this session.** Present, in one message:
...
3. **The estimated cost** — call `mcp__plugin_pika_pika__estimate_cost` for the Step-7 `mcp__plugin_pika_pika__generate_reference_video` call and quote the result. If the estimate errors, say the cost is unknown and flag that 4K is the top-price tier if that's what's selected — do not silently skip the number.
...
## Step 7 — Generate (Seedance reference-to-video) (`mcp__plugin_pika_pika__generate_reference_video`)
Call `mcp__plugin_pika_pika__generate_reference_video` with the recipe matched to `state.resolution`:
- `provider`: **`seedance`**
...
## Step 8 — Deliver
If the call returns inline, you have the video URL. If it returns `{ task_id, status: running/background }`, poll `mcp__plugin_pika_pika__task_status(task_id)` in a tight loop until `completed` / `failed` / `cancelled`. On completion, return the **video URL** to the user. Note: the output CDN may be egress-blocked for in-session download — so deliver the URL itself rather than trying to fetch the bytes locally.
...
| Speech / audio not preserved or change off-beat | Run BOTH `mcp__plugin_pika_pika__transcribe_audio` (speech + timing) and `mcp__plugin_pika_pika__analyze_media` on the audio (music/SFX/ambience), then describe the dialogue + lip-movement beats and pin the time-coded change to the right audio cue/beat in the prompt so the re-render stays in sync. |
...
## One-liner recap
Extract EVERY frame locally with ffmpeg → tile into 25-per-sheet (5×5) contact sheets and analyze every sheet → understand the audio (transcribe speech + analyze music/SFX/ambience) → write an exhaustive Seedance-faithful prompt that locks the original and time-codes the change to the right audio beat → **agreement gate: show the prompt + recipe (resolution 1080p by default, 4K/720p on request) + `mcp__plugin_pika_pika__estimate_cost` and wait for approval** → re-render the same shot via `mcp__plugin_pika_pika__generate_reference_video` (`seedance`, model matched to resolution, original as `reference_videos`, `@Video1` in the prompt, no `negative_prompt`).
